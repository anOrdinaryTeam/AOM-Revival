...
float shouldApply(GlitchSeed seed){
	...
	vec4 transverseChromatic(vec2 p){
		vec2 destCoord=p;
		vec2 direction=normalize(destCoord-.5);
		vec2 velocity=direction*intensityChromatic*pow(length(destCoord-.5),3.);
		float inverseSampleCount=1./float(sampleCount);
		
		//mat3x2 increments = mat3x2(velocity * 1.0 * inverseSampleCount, velocity * 2.0 * inverseSampleCount, velocity * 4.0 * inverseSampleCount);
		
		vec2 increment1=vec2(velocity*1.*inverseSampleCount);
		vec2 increment2=vec2(velocity*2.*inverseSampleCount);
		vec2 increment3=vec2(velocity*4.*inverseSampleCount);
