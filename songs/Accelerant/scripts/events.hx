...
// <<<<<<<<<<<< Based on https://github.com/AppleHair/HankPsychPort
var randomPhrases:Bool = false;
var hcStep:Int = 0;
var hcLyrics:Array<String> = ['NO', 'INVALID', 'CLOWN CANNOT DIE', 'RE-ENGAGE', 'CORRECTION', 'Reboot.useSeed[sRAND];'];
var phrases:Array<String> = ['IMPROBABLE','HANK!!!','MADNESS',"WHO'S THAT??",'INTERRUPTION','FIGHT ME','INVALID','CORRECTION','CLOWN','OMFG!!!'];
// >>>>>>>>>>>>>

function stepHit() {
    if (randomPhrases && FlxG.random.bool(7))
        customLine(phrases[FlxG.random.int(0, (phrases.length - 1))], 395, 170);
    ...
    switch(curStep) {
        ...
        case 662:
            ...
            customLine('CLOWN ENGAGED!', 395, 170);

            curLaserPoint = 40;
            laser.x = 690;
        case 672: randomPhrases = true;
        case 736:
        ...
        // hellclown
        case 957, 960, 964, 967, 969, 971:
            if (curDiff == 'fucked') {
                customLine(hcLyrics[hcStep], 395, 170);
                hcStep++;
            }
    }
}
